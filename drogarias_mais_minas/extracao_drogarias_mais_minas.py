...
async def extrai_links_marcas():
    ...
    async def extrai_marcas_quantidades(json_file, departamento):
        ...
        for chave, valor in json_file.items():
            if (re.match(r'.*\.facets\.3\.values\(\{\}\)\.\d+$', chave) if departamento in ['medicamentos', 'nutricao-esportiva', 'nossas-vitaminas'] else re.match(r'.*\.facets\.4\.values\(\{\}\)\.\d+$', chave)):
                marca = valor['value']
                quantidade_produtos = valor['quantity']
                if quantidade_produtos < 15:
                    paginas = 1
                else:
                    paginas = quantidade_produtos // 15 if (quantidade_produtos % 15) == 0 else (quantidade_produtos // 15) + 1
                dicionario['Marca'].append(marca)
                dicionario['Paginas'].append(paginas)
                dicionario['Departamento'].append(departamento)
                dicionario['Link'].append(f'https://www.drogariasminasmais.com.br/{departamento}/{marca}?initialMap=c&initialQuery={departamento}&map=category-1,brand')

        return dicionario

    # departamentos disponíveis
    departamentos = ['medicamentos', # 3
                    'dermocosmeticos', # 4
                    'infantil', # 4
                    'higiene-pessoal', # 4
                    'beleza', # 4
                    'nutricao-esportiva', # 3 
                    'alimentos', # 4
                    'nossas-vitaminas'] # 3

    marcas_departamentos = pd.DataFrame()
    for departamento in departamentos:
        req = requests.get(f'https://www.drogariasminasmais.com.br/{departamento}')
...
async def main():
    ...
    json_path = 'drogarias_mais_minas/base_oficial_drogarias_mais_minas.json'
