'''
------------------------------------------------------
SCRIPT PYTHON PARA EXTRAÇÃO DE DADOS DA BELEZA NA WEB.
------------------------------------------------------
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        responses = executor.submit(get_response,urls_individuais).result()

    final_df = pd.DataFrame()
    # 5. extraindo informações das urls das páginas individuais
    i=1
    for response in responses:
        df_temp = extract_info(response)
...
def get_response(urls, tries=3):
    ...
    for i, url in enumerate(urls, start=1):
        print(f'Requisição {i}/{total_urls}')
